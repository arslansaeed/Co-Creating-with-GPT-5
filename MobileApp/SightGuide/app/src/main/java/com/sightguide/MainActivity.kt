...
//@AndroidEntryPoint  // this is to inject dependencies into composables
    ...
    private val requestPermissionsLauncher =
        registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permissions ->
            ...
            if (audioGranted && cameraGranted) {
                ...
                // Potentially start something that needs both permissions
            } else {
                if (!audioGranted) {
                    mainViewModel.onAudioPermissionDenied() // Assuming ViewModel has this method
                    Toast.makeText(this, "Audio permission denied. Voice input unavailable.", Toast.LENGTH_LONG).show()
                }
                if (!cameraGranted) {
                    // Handle camera permission denial, e.g., show a message or disable camera features
